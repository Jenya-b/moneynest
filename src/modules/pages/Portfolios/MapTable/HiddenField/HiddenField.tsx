...
import { setCheckedMapHeadData, setMapHeadData } from 'modules/store/reducers/portfolioSlice';
...
export const HiddenField = () => {
  ...
  useEffect(() => {
    if (!checkedMapHeadData) return;
    const newData = portfoliosheadData.filter((item) => checkedMapHeadData[item]);

    dispatch(setMapHeadData(newData));
  }, [checkedMapHeadData]);

  useEffect(() => {
    setCountHiddenColumns(portfoliosheadData.length - mapHeadData.length);
  }, [mapHeadData]);
        ...
        {portfoliosheadData.map((item) => (
          <MenuItem key={item}>
            <FormControlLabel
              control={
                <Checkbox
                  checked={checkedMapHeadData[item]}
